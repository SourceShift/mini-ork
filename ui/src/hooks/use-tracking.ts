 ...
 *
 * All events require explicit user consent. The shared telemetry client
 * enforces the canonical consent configured by telemetry.ts; this hook must
 * not gate on backend settings because they can be stale while a backend
 * changes. (The upstream agent-canvas SPA wired this through a third-party
 * analytics SDK; for the minio-ork fork, the calls no-op until the
 * minio-ork backend exposes its own telemetry sink.)
 */
export const useTracking = () => {
  ...
  /**
   * The shared telemetry client enforces the canonical consent configured by
   * telemetry.ts. Backend settings are not a capture gate because they can be
   * stale while a backend is being added or switched.
   */
