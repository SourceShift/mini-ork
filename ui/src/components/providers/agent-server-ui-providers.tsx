...
export interface AgentServerUIAnalyticsConfig extends TelemetryConfig {
  provider: "agent-canvas";
}

export type AgentServerUIAnalyticsConfigValue =
  | AgentServerUIAnalyticsConfig
  | false
  | null;

export const DEFAULT_AGENT_SERVER_ANALYTICS: AgentServerUIAnalyticsConfigValue = false;
...
> {
  ...
  analytics?: AgentServerUIAnalyticsConfigValue;
  i18n?: I18nInstance;
...
export function AgentServerUIProviders({
  ...
  const analyticsConfig =
    analytics && analytics.provider === "agent-canvas"
      ? {
      ...
      : false;
  const content = (
    <TelemetryProvider config={analyticsConfig}>{children}</TelemetryProvider>
  );
