...
const appBuildConfig = {
  // The React Router v7 plugin overrides Vite's outDir to manage its own
  // `build/client/` + `build/server/` split, so the prod outDir is
  // redirected by the `buildEnd` hook in react-router.config.ts (moves
  // `build/client/*` to `../mini_ork/web/static/`). emptyOutDir stays
  // off so the build doesn't wipe the static dir while we're mid-move.
  sourcemap: true,
  rolldownOptions: {
    output: {
      codeSplitting: {
...
export default defineConfig(({ mode }) => {
  const {
    VITE_BACKEND_HOST = "127.0.0.1:7090",
    VITE_USE_TLS = "false",
    VITE_FRONTEND_PORT = "7070",
    ...
    server: {
      ...
      proxy: {
        "/api": {
          ...
          // ws:true so the PTY bridge (ws://…/api/v1/pty) and socket.io
          // connections proxy to the API in dev — without it the
          // long-lived sockets 404 against the Vite server.
          ws: true,
        },
        "/server_info": {
          target: API_URL,
