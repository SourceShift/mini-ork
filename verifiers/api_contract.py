#!/usr/bin/env python3
"""Catalog seed: the behavioral API-contract verifier.
...
see ``verifiers/api_contract.observable.example.yaml``) or the ``MO_BEHAV_*``
environment variables. With no observable declared it abstains (UNVERIFIED),
which is the opt-in no-op: nothing runs a behavioral check unless a recipe
configures one.
