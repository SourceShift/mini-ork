"""Unit tests: ``mini_ork.cli.eval`` (bash parity halves removed; formerly vs ``bin/mini-ork-eval``).

Each test drives the Python port's real CLI surface (``python3 -m
mini_ork.cli.eval``) against a SQLite fixture and asserts outputs
semantically. No mocks.

Schema bootstrap: every fixture is initialised by ``db/init.sh`` which
applies all migrations and gives the canonical schema
(``workflow_candidates``, ``workflow_memory.yaml_blob``,
``benchmark_tasks``, ``benchmark_results``). We seed candidate rows with a
synthetic workflow_memory entry so the JOIN
``workflow_candidates→workflow_memory.yaml_blob`` resolves.

Eight cases:
  (a) --help             — exit 0, usage on stdout
  (b) no --candidate     — exit 2, "Usage:" emitted
  (c) unknown candidate  — exit 2, "Candidate not found" stderr block
  (d) --dry-run          — benchmark_list JSON + "[dry-run] ..." line
  (e) happy-path non-dry — exit 0; stdout blocks + workflow_candidates row
                            (status, utility_delta, base_workflow_version_id)
  (f) --suite code_fix dry — benchmark_list filtered by task_class
  (g) stdout header/footer — literal "=== mini-ork eval ===" and
                              "=== eval result ===" blocks
  (h) DB OperationalError — chmod 0444 the DB file, port must print
...
def db(tmp_path_factory):
    """Fresh DB initialised via db/init.sh (canonical schema)."""
...
# ─────────────────────────────────────────────────────────────────────────────
# (b) no --candidate: exit 2, "Usage:" emitted on stdout
# ─────────────────────────────────────────────────────────────────────────────
def test_no_candidate(db):
...
# ─────────────────────────────────────────────────────────────────────────────
# (c) unknown candidate: exit 2 with "Candidate not found" stderr block
# ─────────────────────────────────────────────────────────────────────────────
def test_unknown_candidate(db):
    py_rc, py_out, py_err = _run_py(["--candidate", "ghost-xyz"], db)
    assert py_rc == 2
    assert py_out == ""
    assert "Candidate not found in DB: ghost-xyz" in py_err
    assert "FK gap" in py_err
    assert "mini-ork improve" in py_err


# ─────────────────────────────────────────────────────────────────────────────
# (d) --dry-run: benchmark_list JSON + "[dry-run] ..." line
# ─────────────────────────────────────────────────────────────────────────────
def test_dry_run(seeded_db):
...
# ─────────────────────────────────────────────────────────────────────────────
# (e) happy-path non-dry-run: exit 0; substrings present + DB row
# ─────────────────────────────────────────────────────────────────────────────
def test_happy_path(seeded_db):
    py_rc, py_out, py_err = _run_py(
    ...
    assert f"    candidate: {CANDIDATE_ID}" in py_out
    assert "    tasks_evaluated: 0" in py_out
    assert "    total_utility:   0" in py_out
    assert "    baseline_utility:0" in py_out
    assert "    utility_delta:   0" in py_out
    assert "utility_delta=0" in py_out

    # DB row on stable columns.
    py_row = _row(seeded_db, CANDIDATE_ID)
    assert py_row["status"] == "shadow"
    assert abs(float(py_row["utility_delta"]) - 0.0) <= _FLOAT_TOL
    assert py_row["base_workflow_version_id"] == BASE_VERSION_ID


def test_happy_path_db_row_isolated(tmp_path_factory):
    """Run the port against a FRESH DB so the workflow_candidates row check
    has zero cross-contamination: status='shadow' + utility_delta=0.0 +
    base_workflow_version_id unchanged."""
    home_p = tmp_path_factory.mktemp("py_home")
    db_p = str(home_p / "state.db")
    subprocess.run(
        ["bash", str(REPO / "db" / "init.sh")],
        env={**os.environ, "MINI_ORK_HOME": str(home_p),
             "MINI_ORK_DB": db_p},
        capture_output=True, text=True, check=True,
    )
    _seed_candidate(db_p)
    ...
    assert py_row["status"] == "shadow"
    assert abs(float(py_row["utility_delta"])) <= _FLOAT_TOL
    assert py_row["base_workflow_version_id"] == BASE_VERSION_ID
...
# ─────────────────────────────────────────────────────────────────────────────
def test_dry_run_filtered_suite(seeded_db):
...
# ─────────────────────────────────────────────────────────────────────────────
def test_stdout_header_footer(seeded_db):
    py_rc, py_out, _ = _run_py(["--candidate", CANDIDATE_ID], seeded_db)
    assert py_rc == 0

    assert "=== mini-ork eval ===" in py_out
    assert "=== eval result ===" in py_out
    assert "    candidate:       " + CANDIDATE_ID in py_out  # 7-space pad
    assert "    tasks_evaluated: 0" in py_out
    # trailing utility_delta line is exact
    assert re.search(r"^utility_delta=0$", py_out, re.MULTILINE), (
