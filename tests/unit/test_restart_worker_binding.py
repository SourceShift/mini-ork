...
def _fake_worktree(root):
    ...
    (root / "server" / ".env").write_text(
        'QUEUE_PREFIX=dev_\n'
        '# a comment\n'
        'export REDIS_HOST=REDACTED-INTERNAL-IP\n'
        'REDIS_PORT="6380"\n'
...
def test_start_launches_worktree_watchdog(mod, monkeypatch, tmp_path):
    ...
    assert seen["env"]["REDIS_HOST"] == "REDACTED-INTERNAL-IP"
...
def test_runner_model_aligns_from_readyz(mod, monkeypatch, tmp_path):
    ...
    assert seen["url"] == "http://REDACTED-INTERNAL-IP:7910/readyz"  # trailing slash stripped, /readyz joined
