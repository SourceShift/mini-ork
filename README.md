...
- 47 framework primitives in `lib/` (incl. oracle-hardening libs + `gate_bootstrap.sh` for the v0.3-rc1 central wire-up + `lib/throttle-guard.sh` for provider-throttle classification + `lib/mo_otel.sh` for env-gated OTel span emission, added 2026-06-09/10 + `lib/profile_answerer.sh` for MO_AUTO_ANSWER_PROFILE autonomous-dispatch mode, added 2026-06-12 + `lib/krippendorff_alpha_gate.sh` for Nasser 2026 α<0.4 panel-divergence escalation + `lib/citation_verifier_mechanical.sh` for Sistla 2025 mechanical citation coverage + wireheading check, added 2026-06-13)
