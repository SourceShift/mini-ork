...
- Working dir during session: `/Volumes/docker-ssd/Migration/Development/researcher` (libwit repo, but all work landed in mini-ork repo)
- Branch: `main`
...
- `MINIMAX_API_KEY`, `GLM_API_KEY`, `KIMI_API_KEY`, `DEEPSEEK_API_KEY` — provider keys in `.mini-ork/config/secrets.local.sh` (or `.agentflow/config/secrets.local.sh` per the libwit project's pattern)
