...
**Verdict:** the category is real and shipping as of mid-2026 — but **early and narrow**: one small pure-play pioneer (Distil Labs), one big platform with the best numbers as a side feature (Fireworks), and a wide field of RL / generic-distillation / acquired offerings that get *called* this but aren't, on inspection, the same thing. Research (`2505.17612` + fast 2026 follow-ons) is well ahead of every shipped product. The idea is validated (good — the cheap lane works) but the *exact loop* is already a shipped product — which is why the deck **partners** Distil Labs for it rather than rebuilding it.
...
## What this means for Co-Evolve — and why it backs the "partner, don't build" pivot
*(Re-aligned 2026-07-12 to the narrowed deck: one product = the **verified loop**; distillation is deliberately **not** built — partner **Distil Labs**. Supersedes this doc's earlier "build the distillation ourselves" reading.)*
- **The premise is real — which is exactly why we shouldn't build it.** Distilling agentic behavior into a cheap SLM works (10–30×+ savings), and the "point at your traces → auto-distill → host a cheap drop-in student, 50–90% cheaper" loop is *already a shipped product* (Distil Labs + dltHub, July 2026). A funded incumbent owns this layer. So this research **validates the deck's buy-not-build call:** partner Distil Labs, don't rebuild a distillation layer. That cuts execution risk (one product, not three) — and the layer is **not a moat** anyway.
- **What we own is the layer none of them do — the verified loop.** The two things every distillation pure-play (Distil Labs included) *can't* do are our product:
  1. **Whole-loop scope, not a component.** Every pure-play distills *narrow, recurring sub-tasks* and says an open-ended coding agent is out of scope. We don't sell a better distilled tool — we sell the **orchestrate → cheap-first execute → verify → learn** loop that a Distil-Labs model plugs *into* as the cheap lane.
  2. **The execution-anchored, anti-Goodhart reward.** Distil Labs and Fireworks *manufacture* training signal (synthetic data, generic RFT); our differentiator is a reward scored against **real execution tests** a happy LLM reviewer can't fabricate. That verified signal is what makes the compounding real — and it's the exact complement to a partner's model: **they make the cheap student, we verify and compound what it produces.**
- **The honest boundary (baked into the deck too):** the gain is **bounded, concave, and frontier-ceilinged** — distillation extracts a teacher's latent capability, it doesn't transcend it (`2507.14843`, the "invisible leash"). The moat is the per-customer **verified-outcome data** + switching cost — not the distillation, and not "self-improvement past the frontier."
