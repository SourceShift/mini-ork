...
- `DashboardWizardSurface` — `src/components/REDACTED/onboarding/DashboardWizardSurface.tsx:1` — top-level wizard shell. [CONSENSUS: 2/4]
- `OnboardingSurvey` + `OnboardingSurveyLegacy` — `src/components/REDACTED/onboarding/OnboardingSurvey.tsx:1` — intent survey, secondary path. [CONSENSUS: 2/4]
- `WizardStepper` — `src/components/REDACTED/onboarding/WizardStepper.tsx:1` — step nav bar. [CONSENSUS: 1/4]
- `TopicPickerStep` — `src/components/REDACTED/onboarding/TopicPickerStep.tsx:1`. [CONSENSUS: 1/4]
- `SampleChapterVariantCard` — `src/components/REDACTED/onboarding/SampleChapterVariantCard.tsx:1`. [CONSENSUS: 1/4]
- `EvidenceMeter` — `src/components/REDACTED/onboarding/EvidenceMeter.tsx:1` — evidence gate indicator. [CONSENSUS: 1/4]
- `SteerSectionPopover` — `src/components/REDACTED/onboarding/SteerSectionPopover.tsx:1`. [CONSENSUS: 1/4]
- `SaveAndLandingStep` — `src/components/REDACTED/onboarding/SaveAndLandingStep.tsx:1`. [CONSENSUS: 1/4]
- `VariantGenerationStep` / `VariantReviewStep` — same dir. [CONSENSUS: 1/4]
- `RegisterModal` — `src/components/REDACTED/onboarding/RegisterModal.tsx:1` + Turnstile verify backend. [CONSENSUS: 2/4]
- `SampleChapterWizard` — `src/components/REDACTED/onboarding/SampleChapterWizard.tsx:1` — sample co-write. [CONSENSUS: 1/4]
- Step files: `StepIntent`/`StepProfile`/`StepPlan`/`StepSample`/`StepSteer`/`StepClarify`/`StepLanding`/`StepRefreshConfirm`/`StepFullBook` — `src/components/REDACTED/onboarding/steps/*.tsx:1`. [CONSENSUS: 1/4]
- `wizardMachine` — `src/components/REDACTED/onboarding/wizardMachine.ts:128` — XState/reducer FSM, 14 actions, pure. [CONSENSUS: 2/4]
- `evidenceGate.dwEvidenceMet` — `src/components/REDACTED/onboarding/evidenceGate.ts:22` — boolean gate. Partial unit tests. [CONSENSUS: 1/4]
...
- `ReaderShell` — `src/components/REDACTED/reader/shell/ReaderShell.tsx:1` — orchestrator. [CONSENSUS: 2/4]
...
- `ReaderShellSummaryPanel` — `src/components/REDACTED/reader/shell/ReaderShellSummaryPanel.tsx:92` — AI summary panel, polls active-task at mount, dispatches regenerate+cancel. [CONSENSUS: 2/4]
- `ReaderShellViewModePopover` — view mode. [CONSENSUS: 1/4]
- `MarkdownChrome` — `src/components/REDACTED/reader/MarkdownChrome.tsx:1` — md adapter chrome. [CONSENSUS: 2/4]
- `PdfChrome` — `src/components/REDACTED/reader/PdfChrome.tsx:1` — pdf chrome. [CONSENSUS: 2/4]
- `MarkdownAdapter` — `src/components/REDACTED/reader/shell/adapters/MarkdownAdapter.tsx:16` — tracks progress/heartbeat/manifest, handles empty-state + RTL fallback. [CONSENSUS: 2/4]
- `BookToc` — `src/components/REDACTED/reader/BookToc.tsx:1`. [CONSENSUS: 2/4]
...
- `ReaderProgressStrip` — `src/components/REDACTED/reader/ReaderProgressStrip.tsx:1`. [CONSENSUS: 2/4]
- `ReaderTranslationOverlay` — in-place translation. [CONSENSUS: 2/4]
- `CompareMode` + `PdfCompareMode` — side-by-side compare. [CONSENSUS: 2/4]
- `ShareProofButton` — `src/components/REDACTED/reader/ShareProofButton.tsx:1` — public snippet share. [CONSENSUS: 2/4]
...
### Reader mobile (5)
- `MobileReaderChrome` — `src/components/REDACTED/reader/mobile/MobileReaderChrome.tsx:1`. [CONSENSUS: 1/4]
...
### PDF search (3)
- `PdfSearchOverlay` — `src/components/REDACTED/reader/search/PdfSearchOverlay.tsx:1`. [CONSENSUS: 1/4]
...
- `Composer` (citation-aware) — `src/components/REDACTED/chat/Composer.tsx:1`. [CONSENSUS: 1/4]
...
### Dashboard chrome (4)
- `DashboardNavRail` — `src/components/REDACTED/dashboard/DashboardNavRail.tsx:1` + `Compact.tsx` + `navConfig.ts`. [CONSENSUS: 1/4]
...
5. **Audio surface invisible despite backend** — `server/services/audiobookService.ts:1` + `server/routes/audiobooks.ts:1` ship; no `AudioPlayer.tsx` in `src/components/REDACTED/reader/`, no Play CTA in `ReaderHeader.tsx`/`ChapterSpine.tsx`.
