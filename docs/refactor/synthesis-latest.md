...
- [BUG-010] [CONSENSUS: 1/4 kimi] `VisualizeVizType` union shrinks between shared, service, and call-site layers
  - File: `src/services/derivativeDispatchService.ts:23` (local re-decl drops `'plot'`) + `shared/types/contentArtifacts.ts:55` (canonical) + `src/components/REDACTED/highlighter/MarkdownHighlighterProvider.tsx:598` (mapping omits `'plot'`).
...
- [BUG-017] [CONSENSUS: 2/4 kimi+glm] `PDFMobileSelectionSheet` ignores `disabledActions` API, only forwards legacy `arxivDisabled`
  - File: `src/components/pdf/PDFMobileSelectionSheet.tsx:28-33,52` + `src/components/REDACTED/highlighter/MobileSheet.tsx:61-66` (canonical `disabledActions?: Partial<Record<HighlighterAction, string>>`).
  - Feature: Mobile parity (F4)
...
- [BUG-020] [CONSENSUS: 1/4 kimi] `MarkdownHighlighterProvider` callbacks omit `showToast` from dep arrays
  - File: `src/components/REDACTED/highlighter/MarkdownHighlighterProvider.tsx:447` + `:146-151`.
...
- [BUG-026] [CONSENSUS: 1/4 glm] `MarkdownHighlighterProvider` toolbar lacks shared `AiGenerationControls`
  - File: `src/components/REDACTED/highlighter/MarkdownHighlighterProvider.tsx:881` — renders `HighlighterToolbar` without `contextModeControl`, unlike `UnifiedMarkdownRenderer`.
