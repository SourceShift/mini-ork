...
- `MarkdownChrome` — `src/components/REDACTED/reader/MarkdownChrome.tsx:1` — Chrome around markdown (scroll/highlights/selection toolbar). [CONSENSUS: 1/4] (GLM)
- `PdfChrome` — `src/components/REDACTED/reader/PdfChrome.tsx:1` — Chrome around PDF (margins/highlights/search). [CONSENSUS: 2/4] (GLM + MiniMax)
- `PdfSidebar` + `PdfMarginSidebar` — `src/components/REDACTED/reader/PdfSidebar.tsx:1` — PDF outline + margin annotations. [CONSENSUS: 1/4] (MiniMax)
- `ReaderShell` — `src/components/REDACTED/reader/shell/ReaderShell.tsx:1` — Full reader shell (top bar/spine/panels/ask bar). [CONSENSUS: 1/4] (GLM)
- `ReaderShellSpine` — `src/components/REDACTED/reader/shell/ReaderShellSpine.tsx:1` — Chapter spine TOC sidebar. [CONSENSUS: 1/4] (GLM)
- `ReaderShellAskBar` — `src/components/REDACTED/reader/shell/ReaderShellAskBar.tsx:1` — Inline ask bar (chat). [CONSENSUS: 1/4] (GLM)
- `ReaderShellSessions` — `src/components/REDACTED/reader/shell/ReaderShellSessions.tsx:1` — Session list/switcher panel. [CONSENSUS: 1/4] (GLM)
- `ReaderHighlightPalette` — `src/components/REDACTED/reader/ReaderHighlightPalette.tsx:1` — 5-color palette. [CONSENSUS: 2/4] (GLM + MiniMax)
- `ReaderChatRail` — `src/components/REDACTED/reader/ReaderChatRail.tsx:1` — Side-rail chat. [CONSENSUS: 2/4] (GLM + MiniMax)
- `ReaderFloatingChat` — `src/components/REDACTED/reader/ReaderFloatingChat.tsx:1` — Mobile floating chat overlay. [CONSENSUS: 2/4] (GLM + MiniMax)
- `ReaderProgressStrip` — `src/components/REDACTED/reader/ReaderProgressStrip.tsx:1` — Footer progress %. [CONSENSUS: 2/4] (GLM + MiniMax)
- `ReaderTranslationOverlay` — `src/components/REDACTED/reader/ReaderTranslationOverlay.tsx:1` — In-place translated render. [CONSENSUS: 2/4] (GLM + MiniMax)
- `ReaderSettingsPopover` — `src/components/REDACTED/reader/ReaderSettingsPopover.tsx:1` — Typography/density/language. [CONSENSUS: 1/4] (MiniMax)
- `ChapterZoomControl` — `src/components/REDACTED/reader/ChapterZoomControl.tsx:1` — Zoom-into-chapter. [CONSENSUS: 2/4] (GLM + MiniMax)
- `ChapterZoomTreeRail` + `ChapterZoomSummaryList` — `src/components/REDACTED/reader/ChapterZoomTreeRail.tsx:1` — 4-level zoom drilldown. [CONSENSUS: 1/4] (MiniMax)
- `BookToc` — `src/components/REDACTED/reader/BookToc.tsx:19` — Collapsible TOC with generation status ticks. [CONSENSUS: 2/4] (GLM + MiniMax)
- `CompareMode` — `src/components/REDACTED/reader/CompareMode.tsx:1` — Side-by-side chapter compare. [CONSENSUS: 2/4] (GLM + MiniMax)
- `PdfCompareMode` — `src/components/REDACTED/reader/PdfCompareMode.tsx:1` — PDF compare. [CONSENSUS: 2/4] (GLM + MiniMax)
- `ShareProofButton` — `src/components/REDACTED/reader/ShareProofButton.tsx:1` — Public-link share. [CONSENSUS: 2/4] (GLM + MiniMax)
- `TimeToMasteryCounter` — `src/components/REDACTED/reader/TimeToMasteryCounter.tsx:1` — Mastery timer. [CONSENSUS: 2/4] (GLM + MiniMax)
- `VerifiedLearningCard` — `src/components/REDACTED/reader/VerifiedLearningCard.tsx:1` — Verified learning achievement card. [CONSENSUS: 2/4] (GLM + MiniMax)
- `SessionsPanel` — `src/components/REDACTED/reader/sessions/SessionsPanel.tsx:1` — Sessions browser. [CONSENSUS: 1/4] (GLM)
- `PdfSearchFloatPill` — `src/components/REDACTED/reader/search/PdfSearchFloatPill.tsx:1` — PDF search input. [CONSENSUS: 1/4] (GLM)
- Mobile reader shell — `src/components/REDACTED/reader/mobile/MobileReaderAskSelectionSheet.tsx` + `MobileReaderCiteSheet.tsx` + `MobileReaderFindSheet.tsx` + `MobileReaderHighlightsSheet.tsx` + `MobileReaderTOCSheet.tsx`. [CONSENSUS: 2/4] (GLM + MiniMax)
...
- `HighlighterToolbar` + `HighlighterHoverToolbar` + `ColorPalette` — `src/components/REDACTED/highlighter/` — Hover toolbar + 5-color palette. [CONSENSUS: 1/4] (MiniMax)
- `MobileSheet` — `src/components/REDACTED/highlighter/MobileSheet.tsx:1` — Mobile highlighter sheet. [CONSENSUS: 1/4] (MiniMax)
...
- `DashboardWizardSurface` — `src/components/REDACTED/onboarding/DashboardWizardSurface.tsx:1` — 8-step wizard host; route `/:lang/today?wizard=open`; anon-allowed. [CONSENSUS: 2/4] (GLM + MiniMax)
- `OnboardingSurvey` + `OnboardingSurveyLegacy` — `src/components/REDACTED/onboarding/OnboardingSurvey.tsx:1` — First-touch interview. [CONSENSUS: 2/4] (GLM + MiniMax)
- `SampleChapterVariantCard` — `src/components/REDACTED/onboarding/SampleChapterVariantCard.tsx:1` [CONSENSUS: 2/4] (GLM + MiniMax)
- `VariantGenerationStep` — `src/components/REDACTED/onboarding/VariantGenerationStep.tsx:1` [CONSENSUS: 2/4] (GLM + MiniMax)
- `VariantReviewStep` — `src/components/REDACTED/onboarding/VariantReviewStep.tsx:1` — Review/rate variants; submits section feedback (`SampleChapterWizard.tsx:112` → POST `/section-feedback`). [CONSENSUS: 3/4] (GLM + MiniMax + Codex flow)
- `SaveAndLandingStep` — `src/components/REDACTED/onboarding/SaveAndLandingStep.tsx:1` [CONSENSUS: 2/4] (GLM + MiniMax)
- `TopicPickerStep` — `src/components/REDACTED/onboarding/TopicPickerStep.tsx:1` [CONSENSUS: 2/4] (GLM + MiniMax)
- `SteerSectionPopover` — `src/components/REDACTED/onboarding/SteerSectionPopover.tsx:1` [CONSENSUS: 2/4] (GLM + MiniMax)
- `EvidenceMeter` — `src/components/REDACTED/onboarding/EvidenceMeter.tsx:1` [CONSENSUS: 2/4] (GLM + MiniMax)
- `InlineHint` — `src/components/REDACTED/onboarding/InlineHint.tsx:1` [CONSENSUS: 2/4] (GLM + MiniMax)
- `IntentModeChip` — `src/components/REDACTED/onboarding/IntentModeChip.tsx:1` [CONSENSUS: 1/4] (GLM)
- `RegisterModal` — `src/components/REDACTED/onboarding/RegisterModal.tsx:1` [CONSENSUS: 1/4] (GLM)
- `WizardStepper` — `src/components/REDACTED/onboarding/WizardStepper.tsx:1` [CONSENSUS: 1/4] (GLM)
- `WizardErrorModal` — `src/components/REDACTED/onboarding/WizardErrorModal.tsx:1` [CONSENSUS: 1/4] (GLM)
- `WizardLoader` — `src/components/REDACTED/onboarding/WizardLoader.tsx:1` [CONSENSUS: 1/4] (GLM)
- `wizardMachine` + `evidenceGate` — `src/components/REDACTED/onboarding/wizardMachine.ts:34` — State machine gating step 4 on planning convergence; `canAdvance` enforces `FEATURE_FLAGS.ONBOARDING_GATE_AT`. [CONSENSUS: 1/4] (MiniMax)
- Step bodies (StepIntent/StepClarify/StepProfile/StepPlan/StepLanding/StepRefreshConfirm/StepSample/StepSteer/StepFullBook) — `src/components/REDACTED/onboarding/steps/`. [CONSENSUS: 2/4] (GLM + MiniMax)

### Dashboard (today)
- `TodayPage` — `src/pages/TodayPage.tsx:1` + `TodayPageMobile.tsx` — Default landing. [CONSENSUS: 1/4] (MiniMax)
- `DashboardNavRail` + `DashboardNavRailCompact` + `navConfig` — `src/components/REDACTED/dashboard/DashboardNavRail.tsx:1` — Left rail. [CONSENSUS: 1/4] (MiniMax)
- `NowReadingCard` + `GuestSectionPreview` — `src/components/REDACTED/dashboard/NowReadingCard.tsx:1` [CONSENSUS: 1/4] (MiniMax)
- `ProjectPicker` + `ProjectColorSwatch` — `src/components/REDACTED/ProjectPicker.tsx:1` [CONSENSUS: 1/4] (MiniMax)
...
- `CitationFilterPopover` + `CitationRow` + `BibliographyCard` + `TopConceptsCard` — `src/components/REDACTED/citations/` [CONSENSUS: 1/4] (MiniMax)
...
- `Composer` + `FollowupChips` + `ScopeChips` + `SourcesRail` + `CitationCard` + `MessageBubble` + `AssistantMarkdownContent` — `src/components/REDACTED/chat/` [CONSENSUS: 1/4] (MiniMax)
...
- `FeedbackThumbs` (atom) — `src/components/REDACTED/atoms/FeedbackThumbs.tsx:1` + `src/components/REDACTED/feedback/FeedbackThumbs.tsx:45` (shared button invoking `onFeedback`). [CONSENSUS: 2/4] (MiniMax + Codex)
...
- `AuthGate` — `src/components/REDACTED/auth/AuthGate.tsx:1/8` — Wraps `/:lang/*`; `anonAllowedMatcher` carve-out for dashboard wizard. [CONSENSUS: 1/4] (MiniMax)
...
- `wizardMachine` / `evidenceGate` / `wizardSteps` — `src/components/REDACTED/onboarding/wizardMachine.ts:34` [CONSENSUS: 1/4] (MiniMax)
...
- `FEATURE_*` flags (20+) — Enumerated in `server/config/features.ts` and `featureFlags.ts`; flip via SealedSecret on REDACTED-INTERNAL-HOST. [CONSENSUS: 1/4] (MiniMax)
...
- **Translation overlay** — GLM lists `ReaderTranslationOverlay` at `src/components/REDACTED/reader/ReaderTranslationOverlay.tsx:1`. MiniMax §"Coverage gap" #4 says it's "one-shot... no per-paragraph toggle, no show-original-on-hover, no A/B compare." Resolution: component shipped, advanced interaction states missing.
