...
- **★ Chart flag-mirroring** — both lenses independently ran the same
  ...
  anchors (`infra/charts/REDACTED/values.yaml:299` `BCE_M3_CALIBRATOR`, `:307`
...
|---|---|---|---|---|---|
| 1 | **O-R1 ★** | `FEATURE_LLM_JUDGE_CRON` absent → GEPA promotion starves | Opus (+ Codex D-4 corroborates the component) | Add the flag (+ confirm `concludeShadowCron` gate) to `infra/charts/REDACTED/values.yaml` beside `:307`, with a loud `logger.info` on flag-OFF | 0.5 d |
...
|---|---|---|---|
| `FEATURE_PROMPT_EVOLUTION` | ✅ | `infra/charts/REDACTED/values.yaml:307` | OK |
| `BCE_M3_CALIBRATOR` | ✅ | `values.yaml:299` | OK |
| `BCE_M1_MEM0` | ✅ | sealed `infra/charts/REDACTED/templates/sealed/libwit-server-env.yaml:42` | OK |
