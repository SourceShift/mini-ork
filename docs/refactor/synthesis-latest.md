...
|---|---|---|---|---|---|
| 1 | **D-14 ★ O-R3** | Mirror mem flags to the auditable chart surface | codex+opus | Add `BCE_M1_MEM0`, `OPENMEMORY_HOST`, `FEATURE_MEMORY_HARD_FAIL`, `FEATURE_LLM_JUDGE_CRON` to `infra/charts/REDACTED/values.yaml` `backend.env` with explicit on/off (host is not a secret) | 0.5 d |
...
| 2 | `BCE_M1_MEM0` / `OPENMEMORY_HOST` (whole Mem0 triad gate) | gate `bookMemoryService.ts:191` | **NO on auditable surface** — sealed only | `grep -c 'BCE_M1_MEM0\|OPENMEMORY_HOST' infra/charts/REDACTED/values.yaml` → **0**; present only at `sealed/libwit-server-env.yaml:42,145` | D-14 ★ O-R3 |
