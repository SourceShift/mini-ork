...
**Falsifiable smoke recipe against `203.0.113.10:7825`:**
...
2. **Loki check** (`203.0.113.10:13101`): `{app="libwit-backend"} |= "skipping viz research plan for page insertion"` — expect one line per run on both paths (confirms preview-only premise).
