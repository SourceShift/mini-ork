...
peer modules ``mini_ork.registries.coord_registry`` and
``mini_ork.gates.coord_gate``.
...
def _captured_streams() -> Iterator[tuple[io.StringIO, io.StringIO]]:
    ...
    ``mini_ork.registries.coord_registry`` writes via ``sys.stdout.write`` /
    ``sys.stderr.write`` (matches bash heredoc behaviour) rather than
    returning its outputs as a tuple. ``mini_ork.gates.coord_gate`` does
