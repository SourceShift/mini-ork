...
``mini_ork.learning.benchmark_suite`` so we don't re-port that surface.
...
from mini_ork.learning import benchmark_suite
...
def _safe_trace_write(payload: str) -> None:
    ...
    """
    try:
        from mini_ork.stores import trace_store
    except ImportError:
        return
    try:
