...
Pipeline map (bash subcommand → Python dispatch):
  sweep                 → bug_report_sweep      (delegates to mini_ork.ported.bug_report)
...
from mini_ork.ported.bug_report import (
    bug_report_list,
...
def _resolve_root() -> Path:
    ...
    Resolution order:
      ...
      2. Parent of the ``mini_ork`` package (``mini_ork/ported/_this_.py``
         → ``mini_ork/ported`` → ``mini_ork`` → REPO). ``Path.resolve()``
         canonicalizes symlinks, mirroring ``readlink -f``.
    """
    env_root = os.environ.get("MINI_ORK_ROOT")
    if env_root:
        return Path(env_root).resolve()
    # mini_ork/ported/mini_ork_bugs.py → mini_ork/ported → mini_ork → REPO
