...
_check "artifact-diff-shape" "framework-edit.diff contains unified diff anchors" \
  'grep -qE "^(diff --git|--- |\+\+\+ |@@ )" "$DIFF"'
_check "evidence-log-opened" "evidence log path is writable" '[ -w "$EVIDENCE" ]'
...
_check "patched-copy-created" "diff applies to throwaway copy for static checks" \
  '_make_patched_copy'
_check "shell-syntax-clean" "changed .sh files pass bash -n" \
  'ok=1; while IFS= read -r f; do case "$f" in *.sh) [ -f "$WORKTREE/$f" ] && bash -n "$WORKTREE/$f" || ok=0;; esac; done <"$CHANGED"; [ "$ok" = 1 ]'
_check "python-compile-clean" "changed .py files compile" \
  'ok=1; while IFS= read -r f; do case "$f" in *.py) [ -f "$WORKTREE/$f" ] && python3 -m py_compile "$WORKTREE/$f" || ok=0;; esac; done <"$CHANGED"; [ "$ok" = 1 ]'
_check "typescript-typecheck-or-explicit-skip" "run pnpm ui typecheck when TS/TSX files changed" \
  'if grep -qE "\.(ts|tsx)$" "$CHANGED"; then pnpm --dir "$WORKTREE/ui" typecheck; else echo "skip: no ts/tsx files changed" >&3; fi'
_check "high-blast-radius-guard" "high-blast-radius paths require exact ALLOW_HIGH_BLAST_RADIUS token" \
...
_check "migration-pairing" "changed migrations use numbered .sql filenames" \
  'bad=0; while IFS= read -r f; do case "$f" in migrations/*.sql) [[ "$f" =~ ^migrations/[0-9]{3,}_.+\.sql$ ]] || bad=1;; esac; done <"$CHANGED"; [ "$bad" = 0 ]'
