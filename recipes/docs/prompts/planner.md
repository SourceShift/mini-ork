...
1. **Every decomposition step MUST declare `node_type: "implementer"`.**
   The framework's D-008b plan-validation gate (shared across recipes)
   rejects plans whose `decomposition[].node_type` is missing or not in
   the canonical set `{planner|researcher|implementer|reviewer|verifier|reflector|publisher|rollback}`.
   For the `docs` recipe every edit step is dispatched to the
